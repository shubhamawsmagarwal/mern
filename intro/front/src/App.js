...
class App extends Component{
  ...
  getDataFromDb(){
    axios.get('http://:3001/get').then((res) => {this.setState({ data: res.data })}).catch(err=>{console.log(err)});
  }
  putDataToDB(id,message){
    this.setState({ loading: true });
    axios.post('http://:3001/add', {id: parseInt(id),message: message}).then((res) => {this.setState({ data:[...this.state.data,res.data]});this.setState({ loading:false });}).catch(err=>{console.log(err)});
  }
  deleteFromDB(id){
